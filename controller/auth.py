...
class PolicyController:
    ...
    def __init__(self, config: dict = DB_CONFIG, sql_dir: str = SQL_DIR) -> None:
        if config is None or sql_dir is None:
            auth_log.fatal(msg=f'Failed to create policy controller, the args are: {DB_CONFIG}, {SQL_DIR}')
    ...
    def map_credentials(self, login: str, password: str) -> str or None:
        ...
        auth_log.debug(msg=f'Maps credentials')
        ...
        try:
            ...
            auth_log.warning(msg=f'Failed to map credentials')
            return
